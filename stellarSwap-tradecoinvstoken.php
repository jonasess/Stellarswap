...
<script src="include/sendpaymentsbacktocoin.js" ></script>
<?php 
					   ...
					   $stillneeded==false;
						if($num_of_rows>0){
							...
							while (($row = $result->fetch_assoc()) && $getout==false) {
								...
								$coinsendrpc=initconnectionrpc($bidtbuycoincode);
								$coinsendrpc->walletpassphrase(getpassphrase($bidtbuycoincode),60);
								$results=$coinsendrpc->sendfrom($coinkeyseller,$bidtcoinkeybuyer,floatval($bidtbuycoinmoun));
									if(!empty($coinsendrpc->error)){
										$transactionstatus= $coinsendrpc->error;
										$getout=true;
										$stillneeded=true;
									...
									$coinsendrpc=initconnectionrpc($bidtbuycoincode);
									$coinsendrpc->walletpassphrase(getpassphrase($bidtbuycoincode),60);
									$results=$coinsendrpc->sendfrom($coinkeyseller,$bidtcoinkeybuyer,floatval($sellcoinamount));
									if(!empty($coinsendrpc->error)){
										$transactionstatus= $coinsendrpc->error;
										$getout=true;
										$stillneeded=true;
						...
						if($stillneeded==false){
							...
							if ($stmtinsertend = $conn->prepare("INSERT INTO swcoinvstokenaskstable (stseckeyseller,stpubkeyseller,sellcoincode,
								buytokencode,buytokenissuer,coinkeyseller,sellcoinamount,buytokenamoun,pricewithcoin) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
						 
							// Bind the variables to the parameter as strings. 
							//s means string ss means two string .. etc .... protected from sql injection
							$stmtinsertend->bind_param("sssssssss", $stseckeyseller, $stpubkeyseller, $sellcoincode, $buytokencode, $assetissuer, $coinkeyseller,$sellcoinamount,$buytokenamoun,$pricewithcoin);//
					   ...
					   $stillneeded==false;
						if($num_of_rows>0){
							...
							while (($row = $result->fetch_assoc()) && $getout==false) {
								...
								if($buycoinamount>=$asktsellcoinamount){
									...
									$coinsendrpc=initconnectionrpc($buycoincode);
									$coinsendrpc->walletpassphrase(getpassphrase($buycoincode),60);
									$results=$coinsendrpc->sendfrom($asktcoinkeyseller,$coinkeybuyer,floatval($asktsellcoinamount));
									if(!empty($coinsendrpc->error)){
										$transactionstatusb= $coinsendrpc->error;
										$getout=true;
										$stillneeded=true;
									...
									$coinsendrpc=initconnectionrpc($buycoincode);
									$coinsendrpc->walletpassphrase(getpassphrase($buycoincode),60);
									$results=$coinsendrpc->sendfrom($asktcoinkeyseller,$coinkeybuyer,floatval($buycoinamount));
									if(!empty($coinsendrpc->error)){
										$transactionstatus= $coinsendrpc->error;
										$getout=true;
										$stillneeded=true;
						...
						if($stillneeded==false){
							if ($stmtinsertend = $conn->prepare("INSERT INTO swcoinvstokenbidstable (stseckeyseller,stpubkeyseller,selltokencode,
								selltokenissuer,buycoincode,coinkeybuyer,selltokenamount,buycoinmoun,pricewithcoin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
							// Bind the variables to the parameter as strings. 
							//s means string ss means two string .. etc .... protected from sql injection
							$stmtinsertend->bind_param("sssssssss", $stseckeyseller, $stpubkeyseller,$selltokencode,$assetissuer,$buycoincode,$coinkeybuyer,$selltokenamoun,$buycoinamount,$pricewithcoin );//
						 ...
						 <article>
							 ...
							 <article>  
							  <form method="post" action="stellarSwap-tradecoinvstoken.php">
								...
								<input type="submit" class="link-style" id="submitbuy" value="Submit">
								</form>
							 </article>
						 </div> 
